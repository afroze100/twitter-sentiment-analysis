...
############################
    ...
    for i in range(0,len(training_labels)):
        if training_labels[i]=="1":
            words_pos_train+=len(training_tweets[i])
        else:
            words_neg_train+=len(training_tweets[i])
...
############################
    ...
    for i in range(0,len(training_tweets)):       ## list_score_sum
        tweet=training_tweets[i]
        score=0
        for word in tweet:
            if word in dict_final_sum.keys():
                score+=dict_final_sum[word]
            if word in dict_final_lexicon.keys():
                list_score_lexicon[i]+=math.fabs(dict_final_lexicon[word])
        list_score_sum.append(score)

##
##    for i in range(0,len(test_tweets)):       ## list_score_lexicon
##        if list_score_lexicon[i]!=0:
##            list_score_lexicon[i]=float(list_score_lexicon[i])/len(test_tweets[i])
##        else:
##            list_score_lexicon[i]=0




    for i in range(0,len(training_tweets)):       ## for list_score_bayes 
        tweet=training_tweets[i]
        pos=1.0
        neg=1.0
        for word in tweet:
        ...
        pos=pos*float(len_pos)/len(list_tweets_5)
        neg=neg*float(len_neg)/len(list_tweets_5)
        if pos==0 and neg==0:
...
#############################
    ...
    for index in range(0,len(training_tweets)):
        final_list.append([str(list_score_sum[index]),str(list_score_bayes[index]),str(list_score_lexicon[index]),training_labels[index]])

    for ind in range(0,len(training_tweets)):
   ...
   return final_list,training_tweets,test_tweets,dict_vocab_2
##    return list_score_sum,list_score_bayes
